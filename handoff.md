...
| SAMPLE-019 | `RectangleCollisionSample_4_0` / `samples/RectangleCollision` | Exact 19-file source, unchanged XNA game and byte-identical official XNBs freshly rebuilt. Original and native 180-second captures show 34/54 red rectangle hits, including 15/26 frames with no estimated opaque-pixel contact; no unambiguous rectangle overlap stays blue. Release OPENGLES3 and non-threaded WEBGL2 plus the exact gallery copy pass input, collision and error gates. Player position and opaque sprite pixels match XNA at start and both clamps. No sample/CNA/sharp-runtime code fix or workaround; the old title discrepancy is resolved by existing assembly metadata. Canonical products refreshed, named work trees retained pending explicit prune authorization. | `RectangleCollision.html`, local only pending explicit push request |
...
## Next ordered item: SAMPLE-020

The owner explicitly chose to skip `SAMPLE-015` for now. `SAMPLE-016` Bounce,
`SAMPLE-017` Collision, `SAMPLE-018` PerPixelCollision and `SAMPLE-019`
RectangleCollision are `✅` after fresh re-audits. SAMPLE-019's exact source,
fresh original/native/browser evidence, scripts and canonical products are in
`/rv/tmp/samples/SAMPLE-019-RectangleCollisionSample_4_0/`; details are in
`samples/RectangleCollision/missing.md`. Its gallery copy passed the same
Chrome gate. The named work trees remain because pruning was not authorized;
the gallery and audit are committed locally only until the owner explicitly
requests a push. The next numbered row for this sequential pass is `SAMPLE-020`.
