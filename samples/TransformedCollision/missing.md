...
This pass rechecked the **entire 36-file upstream directory** and both runnable
products against the current `cna-samples db9a255`, `cna 40e330fcd` and
`sharp-runtime cb8fd7f8` source chain. The exact `xna4-original/` snapshot
still matches the upstream directory. Both unchanged Windows
Debug/x86/Reach games rebuilt with their own official content projects; all
six XNBs were regenerated byte-identically to the port's checked-in XNBs.
The C# and C++ files, inactive Xbox branches, content declarations, controls
and visuals were rechecked line by line. The no-workaround scan found only
the three preserved `#if !XBOX360` branches in the second product. No sample,
CNA or sharp-runtime code fix, new stub or workaround was required.

Fresh named work trees (`work-xna4-20260919`,
`work-native-opengles3-20260919`, `work-web-webgl2-20260919`) rebuilt both
products. Compilation used at most four concurrent jobs. Original and
Release OPENGLES3 captures each recorded 1800 frames over 180 seconds:

| Measured state | XNA | CNA native |
|---|---:|---:|
| Red per-pixel collision frames | 195 | 182 |
| Unambiguous rectangle overlap left blue | 70 | 37 |
| Marginal rectangle overlap left blue | 12 | 12 |

Every frame where the player could not be located was a red occlusion; neither
run has a red frame with a visible player but no block ink in the player's
rectangle. Both original and native games start at `(343,399)`, clamp at
`(40,399)` and `(727,399)`, and exit on Escape. The second product's
deterministic drag gates and 19-frame pixel-exact native comparison are in
[`../TransformedCollisionTest/missing.md`](../TransformedCollisionTest/missing.md).
Native canonical executables were refreshed and re-smoked; their RUNPATH now
points into the current `libcna/cna` checkout.
