...
public:
    explicit GameplayScreen(bool twoHumans = false) : isTwoHumanPlayers_(twoHumans) {
    ...
    void LoadAssets() {
        ...
        ammoTypeNormalTexture_.emplace(Load<Texture2D>("Textures/HUD/ammoTypeNormal"));
        ammoTypeSplitTexture_.emplace(Load<Texture2D>("Textures/HUD/ammoTypeSplit"));
        ...
        playerOneHealthBarPosition_ = playerOneHUDPosition_ + healthBarOffset;
        playerTwoHealthBarPosition_ = playerTwoHUDPosition_ + healthBarOffset;
        ...
        playerOne_->setNameProperty(isTwoHumanPlayers_ ? "Player 1" : "Player");
        ...
        if (isTwoHumanPlayers_) {
            ...
            playerTwo_ = std::make_shared<AI>(game, spriteBatch);
            playerTwo_->Initialize();
            playerTwo_->setNameProperty("Phone");
        }
    ...
    void Update(GameTime& gameTime, bool otherScreenHasFocus,
        ...
        if ((playerOne_->getCatapultProperty()->getCurrentStateProperty() == CatapultState::Reset ||
            ...
            if (playerOne_->getIsActiveProperty()) {
                ...
                playerTwo_->getCatapultProperty()->setCurrentStateProperty(
                    isTwoHumanPlayers_ ? CatapultState::Idle : CatapultState::Aiming);
            } else {
    ...
    void HandleInput(InputState& input) override {
        ...
        if (input.IsPauseGame(std::nullopt)) {
        ...
        } else if (isTwoHumanPlayers_ && !isFirstPlayerTurn_ &&
            ...
            for (const auto& gestureSample : input.Gestures) {
                if (gestureSample.getGestureTypeProperty() == GestureType::FreeDrag)
                    ...
                    isDragging_ = false;
                std::dynamic_pointer_cast<Human>(playerTwo_)->HandleInput(gestureSample);
            }
        }
    }
...
private:
    ...
    void DrawHud() {
        ...
        spriteBatch.Draw(GetWeaponTexture(*playerOne_),
                         playerOneHUDPosition_ + Vector2(33.0f, 35.0f),
                         Color::White);
        ...
        Rectangle rectangle(
            static_cast<int>(playerOneHealthBarPosition_.X),
            static_cast<int>(playerOneHealthBarPosition_.Y),
            static_cast<int>(healthBarFullSize_.X) * playerOne_->getHealthProperty() / 100,
        ...
        spriteBatch.Draw(GetWeaponTexture(*playerTwo_),
                         playerTwoHUDPosition_ + Vector2(33.0f, 35.0f),
                         Color::White);
        ...
        rectangle = Rectangle(
            static_cast<int>(playerTwoHealthBarPosition_.X),
            static_cast<int>(playerTwoHealthBarPosition_.Y),
            static_cast<int>(healthBarFullSize_.X) * playerTwo_->getHealthProperty() / 100,
            static_cast<int>(healthBarFullSize_.Y));
        ...
        if (isFirstPlayerTurn_)
            text = !isDragging_
                ? (isTwoHumanPlayers_ ? "Player 1, " : "") +
                    std::string("Drag Anywhere to Fire")
                : "Release to Fire!";
        else if (isTwoHumanPlayers_)
    ...
    const Texture2D& GetWeaponTexture(const Player& player) const {
        switch (player.getWeaponProperty()) {
        case WeaponType::Normal:
            return *ammoTypeNormalTexture_;
        case WeaponType::Split:
            return *ammoTypeSplitTexture_;
        default:
            throw std::invalid_argument("Player has invalid weapon type");
        }
    }
    ...
    std::optional<Texture2D> ammoTypeNormalTexture_;
    std::optional<Texture2D> ammoTypeSplitTexture_;
    ...
    Vector2 playerOneHealthBarPosition_ = Vector2::Zero;
    Vector2 playerTwoHealthBarPosition_ = Vector2::Zero;
    Vector2 healthBarFullSize_ = Vector2::Zero;
    std::shared_ptr<Human> playerOne_;
    std::shared_ptr<Player> playerTwo_;
    Vector2 wind_ = Vector2::Zero;
    bool changeTurn_ = false;
    bool isFirstPlayerTurn_ = false;
    bool isTwoHumanPlayers_ = false;
