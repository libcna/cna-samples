...
public:
    ...
    Catapult(Game& game, SpriteBatch& screenSpriteBatch, std::string idleTexture,
        ...
        isHuman_ = isHuman;
        projectileStartPosition_ = isLeftSide ? Vector2(630.0f, 340.0f)
    ...
    float getWindProperty() const { return wind_; }
    ...
    float getShotAngleProperty() const { return shotAngle_; }
    void setShotAngleProperty(float value) { shotAngle_ = value; }

    Vector2 getProjectileStartPositionProperty() const { return projectileStartPosition_; }

    int getGroundHitOffsetProperty() const {
        return animations_.at("Fire").getFrameSizeProperty().Y;
    }
    ...
    void Initialize() override {
        ...
        for (const auto& animationDefinition : definitions) {
            ...
            if (!toLoad.has_value() || toLoad.value() == isLeftSide_) {
        ...
        normalProjectile_ = std::make_shared<Projectile>(
            *curGame_, *spriteBatch_, activeProjectiles_, "Textures/Ammo/rock_ammo",
            projectileStartPosition_, static_cast<float>(getGroundHitOffsetProperty()),
            isLeftSide_, Gravity);
    ...
    void Update(GameTime& gameTime) override {
        ...
        case static_cast<int>(CatapultState::ProjectilesFalling):
            ...
            break;
        case static_cast<int>(CatapultState::HitDamage):
            if (!animations_.at("hitSmoke").getIsActiveProperty())
                postUpdateStateChange = CatapultState::Reset;
            animations_.at("hitSmoke").Update();
            break;
        case static_cast<int>(CatapultState::HitKill):
...
private:
    ...
    HitCheckResult CheckHit(const Projectile& projectile);
    void UpdateHealth(Player& player, const BoundingSphere& projectile,
                      const BoundingBox& catapult);
    int getEnemyScore() const;
    void setSelfHealth(int value);
