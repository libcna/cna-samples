...
protected:
    ...
    void UpdateProjectileFlightData(const GameTime& gameTime, float wind, float gravity) {
        UpdateProjectileFlightData(static_cast<float>(
            gameTime.getElapsedGameTimeProperty().getTotalSecondsProperty()), wind, gravity);
    }

public:
    void UpdateProjectileFlightData(float elapsedSeconds, float wind, float gravity) {
        flightTime_ += elapsedSeconds;
        const int direction = isRightPlayer_ ? -1 : 1;
