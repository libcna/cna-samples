...
public:
    ...
    int getWidthProperty() const { return animations_.at("Fire").getFrameSizeProperty().X; }
    int getHeightProperty() const { return animations_.at("Fire").getFrameSizeProperty().Y; }
    ...
    void setDrawOffsetProperty(Vector2 value) {
        projectile_->setDrawOffsetProperty(value);
        drawOffset_ = value;
    }

    float getDrawScaleProperty() const { return drawScale_; }
    void setDrawScaleProperty(float value) {
        projectile_->setDrawScaleProperty(value);
        drawScale_ = value;
    }

    void setScreenProperty(GameplayScreen* value) { screen_ = value; }
    ...
    void Initialize() override {
        ...
        const Vector2 projectileStartPosition = isAI_
            ? catapultPosition_ + Vector2(30.0f, -8.0f)
            : catapultPosition_ + Vector2(35.0f, -8.0f);
    ...
    void Update(GameTime& gameTime) override {
        ...
        case static_cast<int>(CatapultState::Firing) |
            ...
            CenterScreenOnPosition(projectile_->getProjectilePositionProperty());
            if (isGroundHit) {
    ...
    void Draw(const GameTime& gameTime) override {
        ...
        case static_cast<int>(CatapultState::Aiming):
        case static_cast<int>(CatapultState::Stalling):
            animations_.at("Aim").Draw(*spriteBatch_, CorrectForCamera(catapultPosition_),
                                        drawScale_, spriteEffects_);
            break;
        case static_cast<int>(CatapultState::Firing):
            animations_.at("Fire").Draw(*spriteBatch_, CorrectForCamera(catapultPosition_),
                                         drawScale_, spriteEffects_);
            break;
        case static_cast<int>(CatapultState::Firing) |
             static_cast<int>(CatapultState::ProjectileFlying):
        ...
        case static_cast<int>(CatapultState::ProjectileHit):
            ...
            animations_.at("fireMiss").Draw(
                *spriteBatch_, CorrectForCamera(projectile_->getProjectileHitPositionProperty()),
                drawScale_, spriteEffects_);
            break;
        case static_cast<int>(CatapultState::Hit):
            animations_.at("Destroyed").Draw(*spriteBatch_, CorrectForCamera(catapultPosition_),
                                              drawScale_, spriteEffects_);
            animations_.at("hitSmoke").Draw(*spriteBatch_, CorrectForCamera(catapultPosition_),
                                             drawScale_, spriteEffects_);
            break;
        case static_cast<int>(CatapultState::Reset):
            DrawIdleCatapult();
...
private:
    ...
    void CenterScreenOnPosition(Vector2 position);

    Vector2 CorrectForCamera(Vector2 value) const {
        return value * drawScale_ + drawOffset_;
    }

    void DrawIdleCatapult() {
        spriteBatch_->Draw(*idleTexture_, CorrectForCamera(catapultPosition_), std::nullopt,
                           Color::White, 0.0f, Vector2::Zero, drawScale_,
                           spriteEffects_, 0.0f);
    }
    ...
    GameplayScreen* screen_ = nullptr;
