...
inline HitCheckResult Catapult::CheckHit(const Projectile& projectile) const {
    ...
    if (sphere.Intersects(selfBox)) return HitCheckResult::SelfCatapult;
    if (sphere.Intersects(enemyBox)) return HitCheckResult::EnemyCatapult;
...
inline void Catapult::HandleProjectileHit(Projectile& projectile) {
    ...
    case HitCheckResult::SelfCatapult:
        if (currentState_ != CatapultState::Hit && currentState_ != CatapultState::Reset) {
            Microsoft::Devices::VibrateController::getDefaultProperty()->Start(
    ...
    case HitCheckResult::EnemyCatapult:
        if (enemy_->getCatapultProperty()->getCurrentStateProperty() != CatapultState::Hit &&
...
inline void Catapult::Fire(float velocity) {
