...
| SAMPLE-039 | `BillboardSample_4_0` | port | **In progress -- ported and running, NOT yet verified.** Content builds through the sample's own `VegetationProcessor`; three campaign firsts: the stock **`XImporter`** (this is a DirectX `.x` model), the **HiDef profile end to end** (Reach refuses the 82 672-triangle mesh part BY DESIGN, and the executable's embedded `RuntimeProfile` resource must say HiDef or the game dies in `LoadContent`), and no `params` array in the pipeline runner (Mono's `mcs` emits `Array.Empty<T>()`, which .NET 4.0 lacks). **Open difference: billboard geometry.** Sky is exactly identical and the terrain horizon agrees to a pixel, but the vegetation is 74.9 % within 8 levels; wind (75.0 %) and the alpha test (72.3 %) are both ruled out, forcing one billboard size gives 87.0 %, and forcing size 40 gives **0.0 %** -- every pixel -- so it is geometry, not filtering. Eliminated with evidence: the vertex declaration, the vertex data read back from the buffer, the shader attribute mapping, the per-part effect parameters, `ModelMesh::Draw`, the generated GLSL's uniform mapping, and determinism. Next: the two-pass `DepthRead`/`NonPremultiplied` compositing over 41 319 overlapping quads. Evidence: `samples/BillboardSample/missing.md`. | 🔎 |
