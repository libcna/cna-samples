...
| SAMPLES-INFRA-005 | 🛠 | Proved the direct Linux-side XNA 4.0 compiler route and, for `SAMPLE-003`, the official `BuildContent` task with the stock FBX, texture, font and effect pipeline for all eight assets under an isolated Wine prefix. Live original-window capture remains environment-dependent. Still extract the reusable workflow, Win7/VS2010 VM fallback and shared capture checklist from the per-sample evidence. |
| SAMPLES-INFRA-006 | 🛠 | Qualified `CNA_GRAPHICS_RENDERER=WEBGL2` with `SAMPLE-001` and `SAMPLE-003` in real Chrome. `SAMPLE-003` exercises original compiled effects, models, fonts, 13 technique changes, five model changes and Escape; its audit also fixed CNA's public Wasm stack contract. Still add reusable local-HTTP/browser smoke automation before making this the shared per-sample web gate. Do not test or claim another renderer. |
...
|---|---|---|
| SAMPLES-DEC-001 — Native/custom `.fx` path | `SAMPLE-003` proves that an official XNA 4.0 Effect XNB with 13 techniques loads and renders through the existing compiled-effect path on OPENGLES3 and WEBGL2; it required no hand translation. Audit later shader samples for constructs outside this proven subset before escalating a large missing subsystem. | If a concrete later effect remains unsupported after retest, choose a reusable faithful ingestion/translation strategy versus the exact bounded alternative supported by evidence. Do not hand-wire effect behavior inside each sample. |
| SAMPLES-DEC-002 — XNA content-pipeline extensibility | `SAMPLE-004` confirms the first concrete case: it has no `Game`, but a 369-line `CompileEffect` CLI using `EffectImporter`/`EffectProcessor` plus a 2,680-line six-effect wrapper library and 2,013 lines of HLSL. The unchanged original source builds and generates all six blobs. CNA OPENGLES3 parses the exact blobs and renders the BasicEffect blob, but CNA/sharp-runtime have no design-time Content Pipeline authoring API or effect-source compiler. Evidence: `samples/StockEffects/missing.md`. | For `SAMPLE-004`, choose: end the audited non-game row and retain its original tool evidence; port only the educational wrapper library with an explicit CLI boundary; or authorize the full reusable compiler/pipeline subset and define what a web authoring artifact means. Do not alias the sample classes to CNA built-ins or invent a game. |
