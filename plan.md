...
| SAMPLES-INFRA-006 | 🛠 | Qualified `CNA_GRAPHICS_RENDERER=WEBGL2` with `SAMPLE-001`, `SAMPLE-003`, all seven `SAMPLE-005` screens and all five original `SAMPLE-006` sprite-effect modes in real Chrome. `SAMPLE-003` exercises original compiled effects, models, fonts, 13 technique changes, five model changes and Escape; `SAMPLE-005` exercises official model/cubemap/skinning content and exposed the ES-3.2 base-vertex assumption plus a remapped-VAO lifetime defect; `SAMPLE-006` exercises pixel-only compiled effects, secondary texture slots and signed normal-map content. Still add reusable local-HTTP/browser smoke automation before making this the shared per-sample web gate. Do not test or claim another renderer. |
