...
| SAMPLE-005 | `ReachGraphicsDemo_4_0` | port | Line-by-line audit complete; all six demos plus title restored with the 22 official XNA-pipeline XNBs and no sample workaround. Fixed CNA base `TextureReader` registration, custom Model Tag ownership and EasyGL semantic vertex binding. Original and OPENGLES3 native runs pass; WEBGL2 build succeeds, but the real-browser run/capture remains pending because this terminal session exposes no connected browser. Evidence: `samples/ReachGraphicsDemo/missing.md`. | 🛠 |
