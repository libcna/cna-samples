...
Windows/HiDef XNA source and content project rebuilt and ran; all nine official XNBs
regenerated byte for byte. The port now preserves the original tap gestures, Phone
fullscreen/30 Hz branch, Xbox left-stick emitter movement, HiDef profile and X-then-Y
random draws for vector acceleration. Historical `help.png` is at the sample root;
loose decoded content was removed, and original non-Content files were restored.
The Xbox and Phone branches also pass syntax compilation; neither platform was run.

Fresh Release native OPENGLES3 and WEBGL2 products run against CNA `9bb6dc0a7`
(also the `next` head) and sharp-runtime `41b918c9`; neither dependency changed.
The original and native products show Explosions, SmokePlume, Emitter and wraparound,
and both close cleanly on Escape. Real Chrome verifies the exact effect names in the
rendered HUD, including a touch-tap transition, with no runtime or HTTP error.
Fresh seeded/frozen XNA-versus-CNA captures score 100.00% of RGB pixels within eight
...
deployment `35498225854` succeeded. The public Particle Sample detail,
page-3 card and both images return HTTP 200; all four public game files are
SHA-256-identical to the Chrome-tested gallery copy. The owner-authorized
prune removed only `work-native-opengles3-20260920/`,
`work-web-webgl2-20260920/`, `xna4-build/obj/` and
`xna4-build/pipeline-runner/`, freeing 288.2 MB. Exact original source,
scripts, evidence, official XNBs, executable and full native/web products
remain. The corrected artifact manifest gives active-checkout restoration
commands with compiled effects enabled and at most four compile jobs. Next:
read-only analysis of `SAMPLE-030`.
...
upstream source and an unchanged Windows XNA Debug/x86/Reach rebuild. Its two
official XNBs regenerated byte-for-byte. Original and Release OPENGLES3
180-second recordings contain 34 and 54 red rectangle hits, respectively,
without an unambiguous rectangle overlap left blue. Fifteen and 26 red frames
have no estimated opaque-pixel contact, confirming this tutorial's behavior is
the opposite of SAMPLE-018's per-pixel test. The player starts and clamps at
identical coordinates in XNA, native and browser, with all 240 opaque sprite
pixels matching the original in each state. Native input, title and clean exit
...
## Sequential re-audit update — 2026-09-19 (SAMPLE-018)

`SAMPLE-018` PerPixelCollision is freshly requalified from its exact 16-file
upstream source and an unchanged Windows XNA Debug/x86/Reach rebuild. Both
official XNBs regenerated byte-for-byte. A 180-second, 1,800-frame original
recording measured 18 red opaque-pixel hits and 12 overlapping-rectangle blue
near misses; the fresh Release OPENGLES3 recording measured 10 and 12, with
zero impossible hits in either. Native keyboard clamps and Escape passed.
Non-threaded Release WEBGL2 and the byte-identical gallery copy passed real
system-Chrome WebGL2, input, collision, HTTP and runtime-error gates. Compilation
...
frames and reports no runtime error. See `samples/AccelerometerSample/{missing,diff}.md`.

`SAMPLE-085` (`AvatarAnimationBlendingSample_4_0`) is freshly audited and `🛑`. The unchanged
Xbox360/HiDef source and official SpriteFont XNB build, and its sample-owned 71-bone blend algorithm
...
`samples/AvatarMultipleAnimations/missing.md`.

`SAMPLE-087` (`AvatarShadows_4_0`) is freshly audited and `🛑` as another distinct product. Its
four Xbox runtime units and all three official LZX XNBs build. It renders 16 independently animated
Avatars twice: normally, then flattened into a full-screen Alpha8 target sampled by the custom
ground effect. CNA's normal Avatar pass is empty, and a real OPENGLES3 test proves EasyGL also
truthfully refuses Alpha8 render targets; WEBGL2 shares that renderer. No substitute using only the
ground or a Color target was added. Decide `SAMPLES-DEC-004`; see
`samples/AvatarShadows/missing.md`.

`SAMPLE-088` is freshly audited and `🛑`. It is a six-unit Phone client that asynchronously loads
a 5x5 Bing imagery plane, pans, switches Aerial/Road and geocodes typed locations. All three exact
XNBs build and five unchanged support units type-check; the exact main source intentionally
`#error`s until a Bing key is supplied. Microsoft has retired free Bing Basic accounts and sunsets
Enterprise in 2028. Sharp Runtime lacks the required WebClient/GeoCoordinate/stream overload, and
WEBGL2 needs an authorized HTTPS credential/CORS policy. This source does not use XmlSerializer;
do not involve the separate `xml` branch. Decide `SAMPLES-DEC-004`; see
`samples/BingMaps/missing.md`.

`SAMPLE-089` (`BingMapsPathFinding_4_0`) is freshly audited and `🛑` as a distinct product.
Its nine runtime units add ordered pushpins, Driving/Walking Bing Routes requests, returned-road
traversal, tank movement/rotation and indexed red route drawing. All five exact Phone/Reach XNBs
...
`SAMPLES-DEC-002`/`005`; see `samples/BitmapFontMaker/missing.md`.

`SAMPLE-091` (`ClientServerSample_4_0`) has a complete and qualified native port, but is `🛑`
under `SAMPLES-DEC-006`. Its unchanged original and exact official XNBs build; the C++ source now
retains the exact create/find/join flow, packet logic, gamer events, inputs and rendering without
the old loose content, F1 overlay or network substitutes. CNA `e20749761` makes native `Join`
complete its real ENet handshake before returning. Debug and Release OPENGLES3 pass two-process
discovery/join, authoritative client input and pixel-identical host/client frames. WEBGL2 builds,
but CNA's browser discovery/hosting path is deliberately empty and the original exposes no direct
address, so a broker/relay or an explicit native-only boundary needs the owner's decision. See
`samples/ClientServerSample/missing.md`.
...
`SAMPLE-076` (`SplitScreenSample_4_0`) is complete. The stale `.model.json` blocker was false:
live CNA loads the authentic official `tank.xnb` with named bones, hierarchy and mesh parents.
Both independent viewports/cameras, all tank bone animation, borders, platform branch and input are
restored; the three checked-in XNBs are byte-identical official output. The unchanged XNA game,
...
zero. SAMPLE-065 used a narrow project containing its exact original Song item because the full
standalone build propagates `Windows Phone` into an unrelated `x86`-only custom pipeline project.
Every XNB has the expected Phone/Windows platform, reader table, stream name and duration; every
WMA is valid stereo WMA v2 and matches that duration. All VM NICs were `none`, no Internet was
used, no credential was stored, and the VM shut down normally. Evidence, logs, checksums and
...
seven-class header and invented F1 overlay are gone; the original decomposition, properties,
static random generator, disposal, cat/dog schedules and logical type names are restored. The
unchanged XNA executable, native OPENGLES3 and real-Chrome WEBGL2 all render the exact content,
record genuine 44.1 kHz stereo output with changing spatial balance, accept camera input and stop
cleanly. WEBGL2 uses a trusted click under Chrome's normal autoplay policy and verifies loop-rest
silence. XNA vs native reaches 96.02/96.31% within eight levels at 2/7 seconds; XNA vs web reaches
96.51%. SharpRuntime commit `eebebd86` adds the missing general `Double` alias and its focused
tests. See `samples/Audio3D/missing.md`.
...
`SAMPLE-091` is complete on native and recorded under `SAMPLES-DEC-006`. Do not start
`SAMPLE-092` until the already-assigned VBox/SongProcessor check and CNA intermediate work are in
a consistent committed state. The next sample in the ordinary queue remains `SAMPLE-092`.

**The XmlSerializer work is READY — the owner released it on 2026-09-01.** It is no longer a
separate `xml` branch to leave alone: it is merged into `sharp-runtime` branch **`next`** at
**`bd282d10`** and pushed. The component is `Xml.Serialization` (`modules/xml-serialization`,
header-only `INTERFACE`, depends on `Core.Base` and `Xml`).

Per the owner's standing instruction, take this **between samples, before starting further
ones**: audit it specifically against `SAMPLE-014`/`SAMPLE-066`/`SAMPLE-070`, fix what a real
port shows to be wrong, and report remaining gaps before changing those ports.

Start with `sharp-runtime/docs/XmlSerializationScope.md` — capability inventory with per-item
evidence, four out-of-scope areas with the greps proving they are unreachable, and seven recorded
deviations. Then read
`modules/xml-serialization/tests/System/Xml/Serialization/RolePlayingGameSaveTests.cpp`, because
the non-obvious part is that **sixteen of the twenty `XmlSerializer` call sites in `Session.cs`
serialize into an already-open `XmlWriter`**, nesting several objects into one
`<rolePlayingGameSaveData>` document rather than each writing its own;
`SerializeInto`/`DeserializeFrom`/`RootElementName` cover that shape.

Two things its author flags as **not** verified: no sample is ported against it, and the types in
its tests are transcriptions from the C# sources rather than real ported classes, so a real port
may hit a shape that was not modelled. Nothing was generated by real .NET and diffed
byte-for-byte — the comparison is against the authentic fixtures shipped in the XNA tree
(ShipGame's six `EntityList`/`LightList` content files, Spacewar's 184-line `settings.xml`),
which is good but is not the same thing.

Fuller handoff, including all seven deviations:
`HANDOFF-2026-09-01-xml-serialization.md` in this repository root — delete it once folded in.

The other items remain listed so they are not rediscovered as surprises,
...
> before any Racing content/model work.** `plan_racing.md`'s GLB-as-canonical-content decision,
> referenced approvingly below, is withdrawn. Canonical source is the original `.X` models through
> the real content pipeline. No glTF/GLB for Racing.
