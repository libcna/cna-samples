...
| # | Name | Source Directory | Reason |
|---|---|---|---|
| 085 | AvatarAnimationBlending | `AvatarAnimationBlendingSample_4_0` | Historical note: the 2026-07-10 owner go/no-go rejected CNA's opt-in substitute Avatar visual as insufficiently faithful. The fresh 2026-09-01 source/build/live-API audit is authoritative for the current campaign: the unchanged Xbox-only original builds, the normal CNA XNA route is intentionally unavailable/no-op, and the `CNAEXT` route is explicitly not the proprietary body. The row is therefore `🛑` under `SAMPLES-DEC-004`, with exact evidence and resume options in `samples/AvatarAnimationBlending/missing.md`; this legacy file does not independently turn that history into a new current-plan completion ruling. |
| 086 | AvatarMultipleAnimations | `AvatarMultipleAnimationsSample_4_0` | Historical grouping under #085 is not a current audit result. The fresh 2026-09-01 audit proves this is a distinct 392-line lesson that composes Celebrate and Wave over different parts of the authentic 71-bone hierarchy; exact source and XNB build, but normal CNA presets/rendering remain zero/unavailable and the substitute extension cannot consume the composed transform list. It is now `🛑` under `SAMPLES-DEC-004`; see `samples/AvatarMultipleAnimations/missing.md`. |
| 087 | AvatarShadows | `AvatarShadows_4_0` | Historical grouping under #085 is incomplete. The fresh 2026-09-01 audit proves a distinct four-unit planar-shadow sample whose exact shader/texture/font XNBs build: 16 bodies are flattened into an Alpha8 render target and composited on the ground. Normal CNA Avatars draw nothing, and real OPENGLES3 evidence independently shows EasyGL lacks truthful Alpha8 targets. It is now `🛑` under `SAMPLES-DEC-004`; see `samples/AvatarShadows/missing.md`. |
...
| 101 | ObjectPlacementOnAvatar | `ObjectPlacementOnAvatarSample_4_0` | Historical grouping under #085 hid a distinct 361-line moving-bone attachment lesson. The fresh 2026-09-01 audit proves the unchanged Xbox source and stock baseball-bat XNB build; the product composes all 71 animation/bind/parent matrices and attaches the model to `SpecialRight`, but normal CNA has no ready body, bind pose or preset data and its extension uses a substitute skeleton. It is `🛑` under `SAMPLES-DEC-004`; see `samples/ObjectPlacementOnAvatar/missing.md`. |
| 088 | BingMaps | `BingMaps_4_0` | Fresh 2026-09-01 correction: this is not a Silverlight/AJAX-control sample. It is a 1,452-line XNA Phone client using Bing Maps REST imagery and Locations endpoints, a 5x5 asynchronous in-memory tile plane and an intentional compile-time key guard. Exact content/support code builds, but free Bing Basic accounts are retired, Enterprise sunsets in 2028, and Sharp Runtime lacks the reusable async HTTP/GeoCoordinate surface. It is now `🛑` under `SAMPLES-DEC-004`; see `samples/BingMaps/missing.md`. |
| 089 | BingMapsPathFinding | `BingMapsPathFinding_4_0` | Fresh 2026-09-01 correction: not a duplicate of #088. It is a distinct 2,612-line Phone route game with ordered pushpins, Driving/Walking REST Routes, returned-road traversal, tank motion and indexed route rendering. Five exact XNBs and eight support units build, but the intentional key guard, retired free Bing service, reusable HTTP/GeoCoordinate/XML gaps and current Azure POST/GeoJSON migration make it `🛑` under `SAMPLES-DEC-004`; see `samples/BingMapsPathFinding/missing.md`. |
